...
# File paths
INPUT_FILE_TRANSACTIONS = 'customer_transactions.xlsx'
INPUT_FILE_PROFILE = 'customer_profile.xlsx'
INPUT_FILE_TWITTER = 'twitter_data.xlsx'
OUTPUT_FILE = 'recommendations.txt'  # Still used for CLI mode
VIDEO_OUTPUT_FILE = 'recommendations_video.mp4'
TEMP_AUDIO_FILE = 'temp_audio.mp3'
TEMP_VIDEO_FILE = 'temp_background_video.mp4'
...
Use complete product names as offered on www.wellsfargo.com. Do not include the customer ID or transaction amounts in the recommendation. Ensure each recommendation is concise and complete, fitting all three within a total of 220 words maximum. Do not truncate the third recommendation; adjust brevity earlier if needed. Do not use any special characters like # or * in the output."""
    ...
    if request.method == 'POST':
        user_prompt = request.form.get('prompt', default_prompt)
        ...
        logger.info(f"Input from browser: {system_prompt} for Customer ID: {customer_id}")
        ...
        if recommendations and "Error" not in recommendations[0]:
            generate_video_with_moviepy(recommendations[0])  # Pass the recommendation text directly
        return render_template_string(HTML_TEMPLATE, prompt=system_prompt, result=None, video_link=None)
    
    return render_template_string(HTML_TEMPLATE, prompt=default_prompt, result=None, video_link=None)
...
def get_recommendation(system_prompt, user_prompt):
    ...
    try:
        ...
        socketio.emit('recommendation', {'content': f"{recommendation}\nVisit wellsfargo.com for more details."})
...
def download_youtube_video(youtube_url, output_path):
    ...
    try:
        ...
        if file_size < 1024:
            logger.warning(f"Downloaded file {output_path} is unusually small ({file_size} bytes), may be incomplete.")
        logger.info(f"Video downloaded to {output_path}, size: {file_size} bytes")
    except Exception as e:
...
def generate_video_with_moviepy(recommendation_text):
    ...
    logger.info(f"Video content: {recommendations_text}")
    
    # Generate audio
    engine = pyttsx3.init()
    engine.setProperty('rate', 140)
    engine.save_to_file(recommendations_text, TEMP_AUDIO_FILE)
    engine.runAndWait()
    
    narration_audio = AudioFileClip(TEMP_AUDIO_FILE)
    ...
    background_base = VideoFileClip(TEMP_VIDEO_FILE).without_audio().resize((1280, 720))
    ...
    background_music = AudioFileClip(BACKGROUND_MUSIC_FILE).subclip(0, audio_duration).volumex(0.15)
    ...
    for temp_file in [TEMP_AUDIO_FILE, TEMP_VIDEO_FILE]:
        ...
        while retries > 0:
            try:
                ...
                logger.warning(f"Failed to delete {temp_file}: {e}. Retrying in 1 second...")
                time.sleep(1)
                retries -= 1
            except Exception as e:
                ...
                raise
        if retries == 0:
            logger.warning(f"Could not delete {temp_file} after retries. File may still be in use.")
    
    logger.info(f"Video generated and saved as {VIDEO_OUTPUT_FILE}")
    socketio.emit('video_link', {'link': f"/static/{VIDEO_OUTPUT_FILE}" if os.path.exists(VIDEO_OUTPUT_FILE) else None})
...
def generate_recommendations(system_prompt, customer_id):
    ...
    try:
        ...
        # Write to file for CLI mode or debugging, but not relied upon for video
        with open(OUTPUT_FILE, 'w') as f:
            for recommendation in recommendations:
                f.write(f"{recommendation}\n")
        
        socketio.emit('recommendations_complete', {'content': "\n\n".join(recommendations)})
...
def generate_all_recommendations_and_video():
    ...
    default_prompt = """You are a Wells Fargo bank financial advisor for the customers. Design the top three dynamic banking products and services recommendations specific to "Consumer and Small Business Banking" category based on customer demographic data, transaction categories, profile, income, expenditure, loan values, profession, and other provided values, along with Twitter sentiment from the provided data. Generate the recommendation as if you are speaking directly to the customer in a powerful, engaging speech to make them interested in buying the products. Follow this exact format:

Start with: Hello [Customer Name], This is from your Investment Advisor [Investment Advisor]
Wellsfargo would like to recommend you few products
Then list exactly three product recommendations.
...
Use complete product names as offered on www.wellsfargo.com. Do not include the customer ID or transaction amounts in the recommendation. Ensure each recommendation is concise and complete, fitting all three within a total of 220 words maximum. Do not truncate the third recommendation; adjust brevity earlier if needed. Do not use any special characters like # or * in the output."""
    ...
    try:
        recommendations = generate_recommendations(default_prompt, customer_id)
        if recommendations and "Error" not in recommendations[0]:
